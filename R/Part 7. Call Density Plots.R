...
# Extract image filenames from directory
TempImagesJahoo <- list.files("data/calldensityplots/Jahoo/WavsMatched/Positive/")
# Split filenames to extract recorder and date information
TempImagesJahoo <- str_split_fixed(TempImagesJahoo, pattern = '_', n = 8)
recorder <- TempImagesJahoo[, 3]
date <- TempImagesJahoo[, 5]
...
TruePositives <- list.files('data/calldensityplots/Jahoo/WavsMatched/Positive')
FalsePositives <- list.files('data/calldensityplots/Jahoo/WavsMatched/Negative')

TempTP <- str_split_fixed(TruePositives,pattern = '.wav',n=2)[,1]
TempTPProb <- str_split_fixed(TempTP,pattern = '_',n=4)[,1]

TempFP <- str_split_fixed(FalsePositives,pattern = '.wav',n=2)[,1]
TempFPProb <- str_split_fixed(TempFP,pattern = '_',n=4)[,1]


1- length(TempFP)/ (length(TempTP)+length(TempFP))

# Calculate precision for Danum -----------------------------------------------------
